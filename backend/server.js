/**
 * Monalisa Flores — Backend seguro de pagamento PIX (MasterPag)
 * ----------------------------------------------------------------
    ...
    req.on("end", async () => {
      ...
      const items =
        Array.isArray(body.items) && body.items.length
          ? body.items
          : [{ title: "Pedido Monalisa Flores", unitPrice: Number(body.amount) || 0, quantity: 1 }];
      ...
      const payload = {
        ...
        paymentMethod: "pix",
        customer: {
          name: customer.name || "Cliente Monalisa",
          email: customer.email || "cliente@monalisaflores.com",
...
server.listen(PORT, () => {
  console.log(`Backend PIX Monalisa rodando em http://localhost:${PORT}`);
});
