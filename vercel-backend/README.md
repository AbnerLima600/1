# 🔐 Backend PIX na Vercel — Monalisa Flores
...
- No fim ele te dá uma URL tipo `https://monalisa-pix.vercel.app`.
...
| Name | Value |
|---|---|
| `MASTERPAG_PUBLIC_KEY` | `pk_live_FPxjs1VlFBs94aGdLnxINorzWwMNxT66` |
...
vercel --prod
```
Anote a URL final de produção (ex.: `https://monalisa-pix.vercel.app`).

### 5. Ligue o site ao backend
No arquivo **`payment-config.js`** do site, troque o `endpoint`:
```js
window.MONALISA_PIX = {
  endpoint: "https://monalisa-pix.vercel.app/api/pix",
